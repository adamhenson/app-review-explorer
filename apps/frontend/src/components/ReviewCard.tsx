  ...
  return (
    <>
      <button
        ...
        aria-label={`Open review: ${review.title || `by ${review.author || 'Anonymous'}`}`}
      >
        <div className='flex items-start justify-between mb-3 gap-3'>
          ...
          <div className='text-sm text-gray-600 truncate flex-shrink min-w-0'>
            by <span className='font-medium'>{review.author || 'Anonymous'}</span>
          </div>
        </div>

        {review.title && (
          <h3 className='text-lg font-semibold text-gray-900 mb-2 line-clamp-1'>{review.title}</h3>
        )}
